...
SELECT 
	...
	ABS(DATEDIFF(YEAR, GETDATE(), mv_employee.birth_date)) AS employee_age, 
	CONCAT_WS(
			' ',
	...
	ABS(DATEDIFF(YEAR, GETDATE(), mv_employee.hire_date)) AS company_tenure_years,
	ABS(DATEDIFF(YEAR, GETDATE(), mv_title.from_date))  AS title_tenure_years,
	ABS(DATEDIFF(YEAR, GETDATE(), mv_department_employee.from_date))  AS department_tenure_years,
	ABS(DATEDIFF(MONTH, GETDATE(), mv_title.from_date)) AS title_tenure_months,
	GREATEST(
...
SELECT 
    ...
    hire_date,
    title,
    LAG(title) OVER (PARTITION BY employee_id ORDER BY effective_date ASC) AS previous_title,
    salary_amount,
    previous_latest_salary,
    LAG(salary_amount) OVER (PARTITION BY employee_id ORDER BY effective_date ASC) AS previous_salary,
    dept_name,
    LAG(dept_name) OVER (PARTITION BY employee_id ORDER BY effective_date ASC) AS previous_department,
    manager_full_name,
    LAG(manager_full_name) OVER (PARTITION BY employee_id ORDER BY effective_date ASC) AS previous_manager,
...
SELECT 
	...
	(CASE 
		...
		THEN CAST(100*(BASE.salary_amount - BASE.previous_latest_salary)/
		BASE.previous_latest_salary AS DECIMAL(10, 2))
		ELSE NULL
	...
	(CASE 
		...
		ELSE NULL
	END)  AS event_name,
	(BASE.salary_amount - BASE.previous_salary) AS salary_amount_change,
	CAST(
		100*(BASE.salary_amount - BASE.previous_salary)/BASE.previous_salary
	AS DECIMAL(10, 2)) AS salary_percentage_change,
	(tenure_benchmark.tenure_benchmark_salary) AS tenure_benchmark_salary,
	CAST(
		100*(BASE.salary_amount - tenure_benchmark.tenure_benchmark_salary)/
		tenure_benchmark.tenure_benchmark_salary
	AS DECIMAL(10, 2)) AS tenure_comparison,
	(title_benchmark.title_benchmark_salary) AS title_benchmark_salary,
	CAST(
		100*(BASE.salary_amount - title_benchmark.title_benchmark_salary)/
		title_benchmark.title_benchmark_salary
	AS DECIMAL(10, 2)) AS title_comparison,
	(department_benchmark.department_benchmark_salary) AS department_benchmark_salary,
	CAST(
		100*(BASE.salary_amount - department_benchmark.department_benchmark_salary)/
		department_benchmark.department_benchmark_salary
	AS DECIMAL(10, 2)) AS department_comparison,
	(gender_benchmark.gender_benchmark_salary) AS gender_benchmark_salary,
	CAST(
		100*(BASE.salary_amount - gender_benchmark.gender_benchmark_salary)/
		gender_benchmark.gender_benchmark_salary
	AS DECIMAL(10, 2)) AS gender_comparison,
	BASE.effective_date,
	BASE.expiry_date
